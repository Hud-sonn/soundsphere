package com.soundsphere.music.utils.cipher
 ...
 * by remote JSON from two independent sources — Faraday first, Zemer as fallback — so rotated
 * players are fixed without an APK update. Parsing/validation is delegated to
 * [PlayerConfigParser]; only validated payloads ever replace the in-memory map or touch the
 * disk cache.
 *
...
object PlayerConfigStore {
    private const val TAG = "Soundsphere_CipherConfig"
    private const val ASSET_NAME = "player_configs.json"

    // Interim first-party-style primary source (to be replaced by a SoundSphere-owned source
    // later — swap this URL only, no structural change needed) with Zemer as the fallback.
    ...
    // (matches Metrolist's own note). Note: names must not start with "player_" —
    // PlayerJsFetcher.writeToCache() purges "player_*" from this shared dir on every player-JS
    // refresh. PlayerJsFetcher.invalidateCache() deletes only player_*/current_hash.txt, so the
    // config cache + ETag survive cipher retries.
     ...
     * remote copies. Cheap (a ~1.5 KB asset + at most two small files) and guarantees configs
    ...
    /**
     * Overlays the last-good cached remote copies from both sources onto the bundled table. On
     * ANY failure to load a source's cache, that cache body AND its meta file are deleted
     * together: an ETag surviving a corrupt/missing body would make every subsequent conditional
     * fetch 304 without a re-download, locking the device on bundled-only configs until the
     * remote content happens to change. (No-op deletes on a clean first run.)
     */
    ...
    /** Merge order: bundled, then Zemer, then Faraday layered on top — Faraday takes priority. */
    ...
    /**
     * Shared fetch tail for the cooldown-gated refresh paths: fetch the Faraday source first,
     * then Zemer only when the specific broken hash being looked up is still missing — Zemer is
     * a genuine fallback, not fetched on every refresh. On a pure network failure (no server
     * reached) reset the caller's cooldown stamp so a rotation hit while briefly offline retries
     * on the next trigger instead of waiting out the cooldown. Returns whether the table changed.
    ...
    /**
     * Fetches one source's remote JSON (with If-None-Match) and applies it when valid. Any
     * failure — HTTP error, network exception, or validation failure — keeps the previous map
     * and cache for that source. lastFetchMs is only advanced on 200/304 so transient failures
     ...
     * Test seam: the no-source overload applies to the Faraday source (the primary).
     */
