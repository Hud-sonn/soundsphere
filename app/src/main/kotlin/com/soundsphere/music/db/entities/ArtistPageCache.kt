...
import com.soundsphere.innertube.pages.ArtistSection
...
fun serializeArtistPage(
    ...
    artist: com.soundsphere.innertube.models.ArtistItem? = null,
): String {
...
fun CachedArtistPage.toArtistPage(): com.soundsphere.innertube.pages.ArtistPage {
    return com.soundsphere.innertube.pages.ArtistPage(
        artist = artist?.let { cached ->
            com.soundsphere.innertube.models.ArtistItem(
                id = cached.id,
                title = cached.title,
        ...
        sections = sections.map { section ->
            ArtistSection(
                ...
                moreEndpoint = section.moreBrowseId?.let { browseId ->
                    com.soundsphere.innertube.models.BrowseEndpoint(
                        browseId = browseId,
...
private fun CachedItem.toYTItem(): YTItem {
    return when (this) {
        is CachedItem.Song -> SongItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.soundsphere.innertube.models.Artist(it.name, it.id) },
            album = album?.let { com.soundsphere.innertube.models.Album(it.name, it.id) },
            duration = duration,
            setVideoId = videoId,
        )
        is CachedItem.Album -> AlbumItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.soundsphere.innertube.models.Artist(it.name, it.id) }.ifEmpty { null },
            year = year,
        )
        is CachedItem.Playlist -> PlaylistItem(
            ...
            author = author?.let { com.soundsphere.innertube.models.Artist(it.name, it.id) },
        ...
        is CachedItem.Episode -> EpisodeItem(
            ...
            explicit = explicit,
            author = author?.let { com.soundsphere.innertube.models.Artist(it.name, it.id) },
            podcast = album?.let { com.soundsphere.innertube.models.Album(it.name, it.id) },
            duration = duration,
            endpoint = null,
        )
    }
